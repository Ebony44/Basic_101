...
public class NoteController : MonoBehaviour
{
    ...
    [HideInInspector] public float assignedBeat;
    ...
    private float missThreshold = 2f;
    ...
    public void Initialize(float beatPosition, SongManager manager)
    {
        ...
        if (songManager.hitLine != null)
            targetPosition = songManager.hitLine.position;
        else
            targetPosition = startPosition + Vector3.down * 10f;
    }
    ...
    public void UpdatePosition(float currentSongBeat)
    {
        ...
        if (beatsTillHit <= 1f && beatsTillHit >= -0.5f && spriteRenderer != null)
        {
